...
def entrainement(fenetre, chemin, encodage,):
	...
	csv.nouveauMot(dictionnaire)
	
	matrice = csv.creerMatrice()
	dictionnaire = csv.creerDictionnaire()
	
	matrice = ent.genConcurrenceMatCSV(text,fenetre,matrice,dictionnaire)
	
	csv.enregistreMatrice(matrice)
...
def recherche(fenetre):
	...
	concurrances = csv.creerMatrice(fenetre)
	if len(concurrances) == 0:
		return 11
	
	#remplir matrice
	matrice = csv.genererMatrice(concurrances,dictionnaire)
...
def main():
	...
	#option pour la recherche
		...
		cls.cluster(int(nbClusters), int(fenetre), listemots, nbMotsAffichage)
		pass
